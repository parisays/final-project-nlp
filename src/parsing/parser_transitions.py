...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            firstWord = self.buffer.pop(0)
            self.stack.append(firstWord)
        elif transition == 'LA':
            secondWord = self.stack.pop(-2)
            self.dependencies.append((self.stack[-1], secondWord))
        elif transition == 'RA':
            firstWord = self.stack.pop(-1)
            self.dependencies.append((self.stack[-1], firstWord))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses)>0:
        ...
        for transition, partialParse in zip(transitions, miniBatch):
