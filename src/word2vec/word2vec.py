...
def naiveSoftmaxLossAndGradient(
    ...
    probabilty = softmax(np.dot(centerWordVec, outsideVectors.T))

    loss = -np.log(probabilty)[outsideWordIdx]
    # outsideWordVector = outsideVectors[outsideWordIdx]
    sumOfX = np.dot(probabilty, outsideVectors)
    gradCenterVec = -outsideVectors[outsideWordIdx] + sumOfX  # -u_o + sum(yhat_x . u_x)

    yHatOutside = probabilty.copy()
    yHatOutside[outsideWordIdx] -= 1
    gradOutsideVecs = np.dot(yHatOutside[:, np.newaxis], centerWordVec[np.newaxis, :])  # v_c (yhat_o - 1)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    centerDrivative = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    negSum = sigmoid(-np.dot(outsideVectors[negSampleWordIndices], centerWordVec))
    loss = -np.log(centerDrivative) -np.sum(np.log(negSum))

    derivative = 1 - negSum
    centerDrivative = 1 - centerDrivative
    gradCenterVec = np.dot(outsideVectors[negSampleWordIndices].T, derivative) - \
                     np.dot(outsideVectors[outsideWordIdx], centerDrivative)

    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = -np.dot(centerWordVec, centerDrivative)
    gradOutsideVecs[negSampleWordIndices] = np.outer(derivative, centerWordVec)

    (unique, counts) = np.unique(indices, return_counts=True)
    for v, k in zip(unique, counts):
        gradOutsideVecs[v] *= k
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        currentLoss, currentGradCenter, currentGardOutside = \
            word2vecLossAndGradient(centerWordVec, word2Ind[outsideWord], outsideVectors, dataset)
        
        loss += currentLoss
        gradCenterVecs[centerIdx] += currentGradCenter
        gradOutsideVectors += currentGardOutside
